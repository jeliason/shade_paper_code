...
fsave <- \(fname,height=6,width=9) {
  ggsave(paste0(figures_folder,fname,".pdf"),device = cairo_pdf, height=height, width=width, units="in")
}
figures_folder <- paste0(path,"figures/simulated_dummy_hier_asymm_poster/")

out <- readRDS(paste0(path,"analysis_simulated_dummy.rds"))
rmse_tb <- out$rmse_tb
sic_tb <- out$sic_tb
rmse_tb <- rmse_tb %>%
...
sic_tb <- sic_tb %>%
  ...
  mutate(num_points_per_type = paste0("Count: ",num_points_per_type)) %>%
  mutate(ratio = paste0("Ratio: ",ratio))
...
rmse_tb %>%
  ...
  stat_halfeye() +
  facet_grid(coefficient~`# of points per type`,scales="free_y",labeller=label_wrap_gen(width = 12)) +
...
rmse_tb %>%
  ...
  stat_halfeye() +
  facet_grid(coefficient~num_points_per_type,labeller = label_wrap_gen(width = 12)) +
  labs(x="Ratio of dummy points to real points",y="Average RMSE") +
  scale_y_continuous(trans="log10") +
  scale_x_continuous(trans="log10") +
  guides(y="axis_logticks",x="axis_logticks") +
  theme(legend.position = "top")
fsave("avg_rmse_over_scales",height=7)
...
sic_tb %>%
  ggplot(aes(x)) +
    ...
    geom_line(aes(y=value,color=Curve)) +
    geom_hline(yintercept=0,linetype="dotted") +
    facet_grid(ratio~num_points_per_type,labeller=label_wrap_gen(width=12),scales="free_y") +
    labs(x="Distance (microns)",y="SIC") +
  theme(legend.position = "top")

fsave("estimated_SIC",height=8)
