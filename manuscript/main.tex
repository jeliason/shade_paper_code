...
Recent advances in multiplexed imaging technologies, including multiplexed immunofluorescence (mIF) and other high-resolution spatial profiling methods, have made it possible to quantify cell type spatial distributions and interactions within the tumor microenvironment (TME) at single-cell resolution~\citep{sheng_multiplex_2023}. \begin{revised}Benchmarking studies have compared classical spatial summary statistics (Ripley's $K$-function, $G$-cross) for detecting co-localization in mIF and IMC data~\citep{soupir_benchmarking_2025}, while functional data analysis methods such as mxfda~\citep{wrobel_mxfda_2024} apply multilevel functional PCA to spatial summary statistics, decomposing variation across hierarchical levels without imposing parametric models on point patterns. While such methods effectively characterize dominant modes of variation in marginal pairwise associations~\citep{diggle_statistical_2013}, they do not model conditional relationships or enable joint estimation with partial pooling at the point process level.\end{revised}

Although Gibbs point process models offer a more flexible probabilistic framework~\citep{moller_statistical_2003, baddeley_spatial_2015}, standard formulations assume symmetric interactions. While hierarchical Gibbs models have been proposed to accommodate directionality~\citep{hougmander_multitype_1999, grabarnik_modelling_2009}, these methods depend on parametric interaction functions that impose restrictive assumptions. \begin{revised}Observations based on the $G$-cross function suggest that spatial interactions between cell types are inherently asymmetric~\citep{tsang_assessing_2024}, motivating statistical models that directly account for directional spatial effects.\end{revised}
