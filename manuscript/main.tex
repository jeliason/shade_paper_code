...
% PLOS format: use "Fig" instead of "Figure" for figure captions
...
\begin{abstract}
\textbf{Motivation:}  
Understanding how different cell types interact spatially within tissue microenvironments is critical for deciphering immune dynamics, tumor progression, and tissue organization. Many current spatial analysis methods assume symmetric associations or \revisedtext{\input{snippets/r2_q1_abstract}}limiting biological interpretability and statistical power. 
...
\begin{revised}Hyperpriors for the variance components \( \sigma_{\text{cohort}}^2 \), \( \sigma_{\text{patient}}^2 \), and \( \sigma_{\text{image}}^2 \) are detailed in \secref{ssec:prior_var}.\end{revised}
