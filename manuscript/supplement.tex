...
\textbf{Envelope Test Comparisons:} We compared SHADE against $G$-cross and $L$-cross envelope tests implemented using the \texttt{spatstat} package. \begin{revised}Global envelopes were constructed using $99$ Monte Carlo simulations of point patterns generated according to complete spatial randomness (CSR). The rank parameters were calibrated in preliminary null scenario simulations to achieve proper Type I error control: \texttt{nrank = 10} for $G$-cross (yielding nominal significance level $\alpha = 10/100 = 0.10$) and \texttt{nrank = 8} for $L$-cross (yielding $\alpha = 8/100 = 0.08$). These nominal levels achieved empirical Type I error rates of 2.5--8.8\% for $G$-cross and 1.3--6.7\% for $L$-cross across different density regimes (Section~\ref{sec:coverage_type1}). Detection success required the observed summary function to deviate outside the simultaneous envelope at any distance, indicating rejection of the null hypothesis of complete spatial randomness.\end{revised}
...
\begin{figure}[h!]
    ...
    \caption{\textbf{Multilevel functional PCA of $G$-cross functions for all source-target pairs.} For each of 15 cell type pairs, group-level mean $G$-cross curves are shown for CLR (blue) and DII (orange) patient groups. Ribbons represent $\pm 1$ SD uncertainty bands from the first functional principal component ($\mu(r) \pm \sqrt{\lambda_1}\phi_1(r)$). Results complement SHADE's conditional SICs by characterizing marginal pairwise spatial associations without multivariate adjustment.}
...
\begin{figure}[h!]
    ...
    \caption{\textbf{Multilevel functional PCA of $L$-cross functions for all source-target pairs.} For each of 15 cell type pairs, group-level mean $L$-cross curves are shown for CLR (blue) and DII (orange) patient groups. Ribbons represent $\pm 1$ SD uncertainty bands from the first functional principal component. The $L$-cross function provides a variance-stabilized alternative to $G$-cross, facilitating comparison across different spatial scales and cell type combinations.}
...
We used the \texttt{mxfda} R package~\citep{wrobel_mxfda_2024} to perform multilevel functional PCA on $G$-cross and $L$-cross functions. For each of the 15 source-target cell type pairs, we computed spatial summary functions separately for CLR and DII patient groups as follows:
...
SHADE and mFPCA address complementary questions about spatial organization:

\begin{itemize}
\item \textbf{Modeling framework:} SHADE provides a generative probabilistic model of the conditional intensity $\lambda(v \mid \mathcal{X}_{A_1}, \ldots, \mathcal{X}_{A_K})$, enabling likelihood-based inference and posterior uncertainty quantification. mFPCA performs dimension reduction on observed summary statistics without modeling the data-generating process.

\item \textbf{Multivariate adjustment:} SHADE estimates conditional effects adjusting for all cell types simultaneously, isolating the unique contribution of each source type. mFPCA analyzes marginal pairwise statistics ($G$-cross, $L$-cross) computed separately for each source-target pair.

\item \textbf{Inference goals:} SHADE targets directional causal effects (``how does proximity to source $A$ affect target $B$ density?''), while mFPCA characterizes dominant modes of functional variation across biological scales.
\end{itemize}
