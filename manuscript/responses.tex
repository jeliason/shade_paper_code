...
\textbf{Response}: We have substantially revised the manuscript to address these concerns.

\textbf{(1) Explicit references:} The Introduction now provides specific citations to common spatial analysis pipelines and hierarchical point process methods:

\begin{revised}
\input{snippets/r2_q1_intro}
\end{revised}

\textbf{(2) Independent analysis and patient heterogeneity:} The Introduction now explicitly explains why independent image-level analysis limits formal study of heterogeneity (see above snippet: ``such workflows cannot perform partial pooling or decompose heterogeneity across biological levels'').

\textbf{(3) SHADE's image-level parameters:} Results Section~\ref{sec:how_vary} clarifies that SHADE does estimate image-level SICs, but within a joint hierarchical framework:

\begin{revised}
\input{snippets/r2_q1_how_vary}
\end{revised}

Simulation evidence in Section~\ref{sec:sim_comparison} demonstrates the benefits of this hierarchical approach:
...
The Discussion further contextualizes SHADE's hierarchical approach relative to traditional spatial statistics:
...
As shown in Section~\ref{sec:gx_comparison}:
...
As shown in Section~\ref{sec:mfpca_comparison}:
...
Section~\ref{sec:sim_comparison} demonstrates that SHADE's performance relative to envelope tests depends on source cell density and the availability of multiple images for hierarchical pooling:
