...
\textbf{Response}: We have clarified the caption for Figure~\ref{fig:example_pred_supp}:
...
\textbf{Response}: The introduction has been substantially revised and streamlined. Method comparisons in the introduction establish the methodological gap and motivation, while detailed comparative analysis with results is included in the supplement (Section~\ref{sec:mfpca_comparison}).
...
\textbf{Response}: We have added both $G$-cross and Ripley's $L$-cross function (transformation of the $K$-function) to our analysis. As now described in Section~\ref{sec:mfpca_comparison} and outlined in the response to the previous question, we implemented a comparison with mxfda~\citep{wrobelMxfdaComprehensiveToolkit2024} (detailed methodology in Supplement Section~\ref{sec:mfpca_methods}):
...
\begin{enumerate}
    ...
    \textbf{Response:} Figure~\ref{fig:gx_detections} (Detection power comparison) has been updated. The current caption is: 
