...
\title{Responses to Reviewers for SHADE}
...
We thank the reviewer for their thoughtful comments, which we believe have greatly improved this manuscript. Your feedback helped us clarify key methodological details and expand our comparisons with existing approaches. Below we have provided point-by-point responses to each comment. For easy reference, all manuscript changes are highlighted in blue, in both the responses here as well as the revised manuscript. \textbf{The blue text in the reply is a verbatim copy of the changes appearing in the paper}. We greatly appreciate the time and care you brought to reviewing this work!
...
\textbf{Response}: This is an important point that we appreciate the opportunity to address. Apparent negative associations at very short distances indeed reflect geometric centroid exclusion rather than biologically meaningful repulsion. We now predefine a minimum interaction radius to avoid attributing geometric crowding to biological interactions, as detailed in Section~\ref{sec:sic}:

\begin{revised}
\input{snippets/r1_q2_crowding_sic}
\end{revised}

To address this further, we have updated the Discussion to note that future extensions could reduce crowding effects using boundary-to-boundary distances:
...
\textbf{Response}: Thank you for catching this oversight. We have now defined all acronyms upon first use in the manuscript. CTLs (cytotoxic T lymphocytes, CD8\textsuperscript{+} T cells) and TAMs (tumor-associated macrophages, CD163\textsuperscript{+}) are now explicitly defined in the Results section.
...
\textbf{Response}: Thank you for pointing this out. We have clarified the caption for Figure~\ref{fig:example_pred_supp}:
...
Simulation evidence in Supplement Section~\ref{sec:sim_hier} and \ref{sec:sim_comparison} now demonstrates the benefits of this hierarchical approach:
...
Additionally, we have added further clarification on the normalization term in Section~\ref{sec:model_setup}:
...
\textbf{Response}: Thank you for highlighting the need for clearer statistical statements. We have made statistical inference much more explicit throughout the manuscript.
...
To address this, Section~\ref{sec:gx_comparison} now presents the comparison:

\begin{revised}
\input{snippets/r2_q4_gx_comparison}
\end{revised}

We have also added Section~\ref{sec:mfpca_comparison} comparing SHADE with mFPCA analysis of $G$-cross and $L$-cross functions:

\begin{revised}
\input{snippets/r2_q4_mfpca_intro}
\end{revised}

To address this, Section~\ref{sec:mfpca_comparison} now presents:
...
\textbf{Response}: This is an excellent point. Our primary simulation does generate data under the same conditional Poisson framework that SHADE assumes, which is standard for initial method evaluation—it allows us to isolate the effects of hierarchical structure, cell density, and sampling variation on estimation accuracy. Even under these ideal conditions, Section~\ref{sec:sim_hier} demonstrates that hierarchical pooling substantially improves inference:
...
\textbf{Robustness to spatial confounding:} To address model misspecification, we added simulations testing SHADE's performance with unmeasured spatial heterogeneity (Section~\ref{sec:compartment_robustness}):
...
\textbf{Response}: Thank you for this helpful organizational suggestion. The introduction has been substantially revised and streamlined. Method comparisons in the introduction establish the methodological gap and motivation, while detailed comparative analysis with results is included in the supplement (Section~\ref{sec:mfpca_comparison}).
...
\textbf{Response}: Thank you for this insightful question about model assumptions. We have expanded Section~\ref{sec:model_setup} to clarify what the inhomogeneous Poisson point process assumption allows and why it is the appropriate modeling choice:
...
\textbf{Response}: Thank you for this excellent question about the relationship between SHADE and functional data analysis approaches. While both SHADE and FDA methods represent spatial relationships as smooth curves using basis function expansions, they differ fundamentally in what they model and how they leverage hierarchical structure. To address this, Supplement Section~\ref{sec:mfpca_methods} now details:

\begin{revised}
\input{snippets/r3_q1_fda_comparison}
\end{revised}

To address this comparison, Section~\ref{sec:mfpca_comparison} now discusses:

\begin{revised}
\input{snippets/r2_q4_mfpca_intro}
\end{revised}

We have also updated the Discussion to contextualize these approaches:
...
\textbf{Response}: Thank you for this constructive suggestion. We have added both $G$-cross and Ripley's $L$-cross function (transformation of the $K$-function) to our analysis. To address this, we implemented a comparison with mxfda~\citep{wrobelMxfdaComprehensiveToolkit2024} as described in Section~\ref{sec:mfpca_comparison} (detailed methodology in Supplement Section~\ref{sec:mfpca_methods}):
...
\textbf{Response}: This is a great question about the relationship between model assumptions. To address this, Section~\ref{sec:model_setup} now clarifies that the inhomogeneous Poisson point process assumption allows:
...
\textbf{Response}: Thank you for this question about study design. To address this, Section~\ref{sec:sim_comparison} now describes:
...
For envelope test calibration (now in Supplement Section~\ref{sec:sim_evaluation_metrics}):
...
\textbf{Response}: Thank you for emphasizing this important check for calibration. We evaluated Type I error rates using null scenario simulations where target cells are generated with no spatial dependence on source cells (all SIC coefficients set to zero). To address this, Section~\ref{sec:sim_comparison} now discusses:
...
\textbf{Response}: Thank you for raising this important practical consideration. The colorectal cancer dataset does consist of tissue microarray (TMA) cores with hundreds to thousands of cells per image. To address computational scalability for larger datasets, we have added timing experiments (Supplement Section~\ref{sec:timing_experiments}), now discussed in Section~\ref{sec:model_estimation}:
...
\textbf{Response}: Thank you for asking us to clarify this aspect of the model. To address this, Section~\ref{sec:multilevel_model} now describes:
...
\textbf{Response}: Thank you for this important question about statistical inference. Under the inhomogeneous Poisson process model, the null hypothesis of "no spatial association" corresponds to SIC = 0 (i.e., the target cell intensity is conditionally independent of the source cell locations). Spatial clustering or repulsion is detected when the estimated SIC is significantly different from zero. To address this, Section~\ref{sec:uncertainty} now describes:
...
Note: The 0.05 effect size threshold previously used to highlight substantive differences in log-intensity has been removed from the analysis.
...
\textbf{Response}: This is an important point about controlling for spatial heterogeneity. The \texttt{mxfda} package only supports standard $G$-cross and not \texttt{Gcross.inhom}, the inhomogeneous version of $G$-cross from \texttt{spatstat}, limiting direct comparison of compartment-adjusted analyses. However, we have added an investigation of the impact of unmeasured compartment structure through simulation (Section~\ref{sec:compartment_robustness}; Supplement Section~\ref{sec:compartment_confounder}):
...
\textbf{Response}: Thank you for this interesting follow-up question. We have added an analysis comparing MAD measures of heterogeneity between CLR and DII patient groups, now discussed in Section~\ref{sec:how_vary}:
...
\textbf{Response}: Thank you for this suggestion to improve the contextualization of our work. We have expanded the Introduction to include a more comprehensive review of spatial co-localization methods applied to multiplexed imaging data in cancer:
...
\begin{enumerate}
    ...
    \textbf{Response:} Thank you for pointing out this visualization issue. Figure~\ref{fig:gx_detections} (Detection power comparison) has been updated. The current caption is: 
    \begin{revised}\input{snippets/fig_gx_detections_caption}\end{revised}
\end{enumerate}
