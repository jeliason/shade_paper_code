...
We thank the reviewers for their thoughtful comments, which we believe have greatly improved this manuscript. Your feedback helped us clarify key methodological details and expand our comparisons with existing approaches. Below we have provided point-by-point responses to each comment. For easy reference, all manuscript changes are highlighted in blue, in both the responses here as well as the revised manuscript (however, we don't ). We greatly appreciate the time and care you brought to reviewing this work!
...
Importantly, the model is fit using all observed cell locations—we do not remove data. Rather, posterior summaries and SICs are evaluated and reported only at distances $r \ge r_{\min}$ for interpretation.
...
\textbf{Response}: We have clarified this throughout the manuscript. We now explicitly state in the Abstract and Introduction that traditional methods:

\begin{revised}
\input{snippets/r2_q1_intro}
\end{revised}

While such methods can describe variability by computing per-image statistics, they do not enable joint estimation with partial pooling.

\textbf{Key distinction:} SHADE does compute image-level parameters ($\delta^{(m,p)}$), but within a hierarchical Bayesian model where these are jointly estimated with patient-level and cohort-level parameters through partial pooling. This is fundamentally different from computing separate summary statistics for each image and then performing post-hoc comparisons.

Results Section~\ref{sec:how_vary} now explicitly contrasts this approach with descriptive spatial statistics:

\begin{revised}
\input{snippets/r2_q1_how_vary}
\end{revised}

Simulation results (Table~\ref{tab:hier_rmse}, Figure~\ref{fig:no_hier_est_SIC}) demonstrate that hierarchical modeling reduces RMSE compared to independent image-level estimation.
...
\textbf{Response}: We have added a detailed explanation of log-intensity in Section~\ref{sec:sic}.

\begin{revised}
\input{snippets/r1_q3_log_intensity}
\end{revised}

Regarding normalization: the intercept term ($\beta_0$) captures baseline log-intensity, effectively normalizing to average cell density. SIC values then quantify deviations from this baseline as a function of proximity to source cells, ensuring curves are directly comparable across images and patients despite differences in overall cell abundance.
...
\textbf{Response}: \joel{Probably need to revisit this} We have made statistical inference much more explicit throughout the manuscript.
...
When bands exclude zero, we have statistical evidence that spatial associations differ from independence. The Bayesian framework automatically propagates uncertainty through all hierarchical levels (image → patient → cohort), accounting for both sampling variability and multilevel variance structure—unlike bootstrap methods that treat each image independently.

\textbf{Clarifying ``statistically predictive'':} We added Section~\ref{sec:predictive} explaining what predictive associations mean:

\begin{revised}
\input{snippets/r2_q3_predictive}
\end{revised}

\textbf{Testing and comparison:} To test departure from spatial randomness, we check if simultaneous bands exclude zero. To compare patient groups, we compute difference curves with simultaneous bands (Figure~\ref{fig:sic_CRC_diff}); if bands exclude zero over a distance range, groups differ significantly. All SIC figures now display these bands, and Results sections explicitly state when bands exclude zero.

\textbf{Figure 4 and CSR:} The figure shows one simulation example. While $G$-cross envelopes contain the observed curve at some distances, SHADE's simultaneous bands correctly exclude zero where ground truth exhibits non-zero interactions. Across all replicates, SHADE achieves high detection rates (Figure~\ref{fig:gx_detections}), successfully distinguishing patterns from CSR through multilevel uncertainty propagation and joint estimation, particularly under low densities where envelope tests may fail.
...
\textbf{Response}: \joel{Also need more details here} We have now added Ripley's cross-type K-function ($K_{AB}$, specifically the $L$-cross transformation) to both the simulation comparison and colorectal cancer analysis.

In the simulation study (Section~\ref{sec:sim_comparison}), we compared SHADE against K-cross envelope tests:

\begin{revised}
\input{snippets/r2_q4_sim_kcross}
\end{revised}

Results (Figure~\ref{fig:gx_detections}) show SHADE outperforms both $G$-cross and $K$-cross in detecting associations under low cell densities and limited sampling.

The Discussion now explicitly contrasts SHADE with traditional spatial summary statistics including Ripley's K-function:

\begin{revised}
\input{snippets/r2_q4_discussion}
\end{revised}

CRC analysis Section~\ref{sec:gx_comparison} compares SHADE with $G$-cross, revealing that TAM–granulocyte associations show strong clustering in $G$-cross but are not emphasized by SHADE's conditional model, while vasculature effects are identified by SHADE as key drivers of CLR vs. DII differences but show limited significance in $G$-cross.
...
Visual comparison (Section~\ref{sec:mfpca_comparison}) reveals both concordance and informative differences. For example, SHADE's vasculature-CTL clustering in CLR patients is corroborated by mFPCA $G$-cross patterns, while CTL-tumor interactions show stronger effects in SHADE due to multivariate adjustment.
...
\textbf{Response}: The reviewer is correct that our primary simulation generates data under the same conditional Poisson framework that SHADE assumes. This is standard for initial method evaluation—it allows us to isolate the effects of hierarchical structure, cell density, and sampling variation on estimation accuracy. Even under these ideal conditions, we demonstrate that hierarchical pooling substantially improves inference and that SHADE outperforms $G$-cross and K-function under sparse data.
...
\textbf{Response}: While both SHADE and FDA methods represent spatial relationships as smooth curves using basis function expansions, they differ fundamentally in what they model and how they leverage hierarchical structure. As detailed in Supplement Section~\ref{sec:mfpca_methods}:

\begin{revised}
\input{snippets/r3_q1_fda_comparison}
\end{revised}

As discussed in Section~\ref{sec:mfpca_comparison}:

\begin{revised}
\input{snippets/r2_q4_mfpca_intro}
\end{revised}

And also in Section~\ref{sec:mfpca_comparison}:
...
\textbf{Response}: We have added both $G$-cross and Ripley's $L$-cross function (transformation of the $K$-function) to our analysis. As described in Section~\ref{sec:mfpca_comparison}, we implemented a visual comparison with mxfda~\citep{wrobel_mxfda_2024} (detailed methodology in Supplement Section~\ref{sec:mfpca_methods}):
...
\textbf{Response}: As described in Section~\ref{sec:model_setup}, the inhomogeneous Poisson point process assumption allows:

\begin{revised}
\input{snippets/r2_minor_poisson_assumption}
\end{revised}

Regarding the logistic regression approximation (Section~\ref{sec:logistic_approx}):
...
\textbf{Response}: As described in Section~\ref{sec:sim_comparison}:
...
For envelope test calibration (Supplement Section~\ref{sec:sim_details}):
...
\textbf{Response}: We agree this is an important check for calibration. We evaluated Type I error rates using null scenario simulations where target cells are generated with no spatial dependence on source cells (all SIC coefficients set to zero). As discussed in Section~\ref{sec:sim_comparison}:

\begin{revised}
\input{snippets/r3_q5_type1_error}
\end{revised}

Full results with figures are presented in Supplement Section~\ref{sec:coverage_type1}.
...
\textbf{Response}: The reviewer is correct that the colorectal cancer dataset consists of tissue microarray (TMA) cores with hundreds to thousands of cells per image. To address computational scalability for larger datasets, we added timing experiments (Supplement Section~\ref{sec:timing_experiments}), discussed in Section~\ref{sec:model_estimation}:
...
\textbf{Response}: As described in Section~\ref{sec:multilevel_model}:
...
\textbf{Response}: Under the inhomogeneous Poisson process model, the null hypothesis of "no spatial association" corresponds to SIC = 0 (i.e., the target cell intensity is conditionally independent of the source cell locations). Spatial clustering or repulsion is detected when the estimated SIC is significantly different from zero. As described in Section~\ref{sec:uncertainty}:

\begin{revised}
\input{snippets/r1_q3_uncertainty}
\end{revised}

When the simultaneous credible band excludes zero over a distance range, we conclude that the spatial association is statistically significant at that range. All SIC figures display simultaneous 95\% credible bands computed using the method detailed in Supplement Section~\ref{sec:sim_band}.
...
\textbf{Response}: We have added an analysis comparing MAD measures of heterogeneity between CLR and DII patient groups, discussed in Section~\ref{sec:how_vary}:
...
\begin{revised}
\input{snippets/r3_q11_lit_review}
\end{revised}
